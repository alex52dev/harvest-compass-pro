...
import { Sprout, TrendingUp, AlertTriangle, Bug, CloudSun, Droplets, Wind, BarChart3, ArrowRight, RefreshCw, ShieldAlert } from "lucide-react";
...
export default function DashboardPage() {
  ...
  const dashQuery = useLatestDashboard(farmId);
  const riskQuery = useRiskAssessmentsByFarm(farmId);
  const pestQuery = usePestPredictionsByFarm(farmId);
  const weatherQuery = useWeatherDataByFarm(farmId);

  const d = dashQuery.data || mockDashboard;
  const risks = riskQuery.data || mockRiskAssessments.filter((r) => r.farmId === farmId);
  const pests = pestQuery.data || mockPestPredictions.filter((p) => p.farmId === farmId);
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      <PageHeader title="Farm Dashboard" description={`Real-time insights for ${farm?.location || "your farm"}`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          { label: "Active Crops", value: farm?.currentCrops?.length || 0, icon: <Sprout className="h-5 w-5" /> },
          { label: "Risk Level", value: risks[0]?.overallRiskLevel || "N/A", icon: <ShieldAlert className="h-5 w-5" />, trend: risks[0]?.overallRiskLevel === "HIGH" || risks[0]?.overallRiskLevel === "CRITICAL" ? "⚠ Action needed" : "Monitoring", trendUp: false },
        ].map((stat, i) => (
        ...
        <motion.div custom={4} initial="hidden" animate="visible" variants={fadeIn} className="bg-card rounded-lg shadow-card border border-border p-5">
          ...
          {weather ? (
            <div className="space-y-3">
              {[
                { label: "Temperature", value: `${weather.temperatureCelsius}°C` },
                { label: "Rainfall", value: `${weather.rainfallMm} mm`, icon: <Droplets className="h-3 w-3" /> },
                { label: "Humidity", value: `${weather.humidityPercent}%` },
                { label: "Wind", value: `${weather.windSpeedKmh} km/h`, icon: <Wind className="h-3 w-3" /> },
              ].map((item) => (
                <div key={item.label} className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground font-body flex items-center gap-1">{item.icon}{item.label}</span>
                  <span className="font-body font-semibold text-card-foreground">{item.value}</span>
                </div>
              ))}
            </div>
          ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        {riskPieData.length > 0 && (
          <motion.div custom={7} initial="hidden" animate="visible" variants={fadeIn} className="bg-card rounded-lg shadow-card border border-border p-5">
            <h3 className="font-heading font-semibold text-card-foreground mb-4">Risk Distribution</h3>
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie data={riskPieData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={80} label={(e) => `${e.name}: ${e.value}%`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: "Risk Assessment", to: "/risk-assessment", color: "gradient-hero" },
          { label: "Recommendations", to: "/recommendations", color: "gradient-green" },
          { label: "Market Insights", to: "/market-insights", color: "gradient-gold" },
        ].map((link, i) => (
