...
export default function FeedbackPage() {
  const { data: apiFeedback, isLoading, isError, refetch } = useFeedback();
  const feedback = apiFeedback || mockFeedback;

  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      <PageHeader title="Farmer Feedback" description="Season reviews and performance ratings">
        <FeedbackFormDialog trigger={<Button size="sm"><Plus className="h-4 w-4 mr-1" /> Submit Feedback</Button>} />
      </PageHeader>

      {isLoading && !apiFeedback ? (
        <CardSkeleton count={3} />
      ) : isError && !apiFeedback ? (
        <ErrorState message="Could not load feedback" onRetry={() => refetch()} />
      ) : feedback.length === 0 ? (
        <EmptyState title="No feedback yet" description="Submit your first season review." />
      ) : (
